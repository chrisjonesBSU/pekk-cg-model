...
class Borah(DefaultSlurmEnvironment):
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument(
                "--gpu",
                default="V100",
                help="Specify the GPU type to use"
        )
...
def sample(job):
    ...
    npt_proj = signac.get_project(
            "/bsuhome/cjones/scratch/pekk-msibi/ua-target-runs/npt-runs-cubic"
    )
    ...
    with job:
        ...
        for _job in npt_proj.find_jobs({"kT_quench": job.sp.kT_quench}):
            job.doc.restart_job = _job.id
            npt_restart = _job.fn("npt-restart.gsd")
        ...
        if job.sp.system_type != "interface":
            ...
            job.doc["total_mass_g"] = system.system_mass * base_units()["amu_to_g"]
        ...
        if sum([1 for i in [
            ...
            print("-----------------------------")
            print("Running NVT quench...")
            print("-----------------------------")
            simulation.quench(kT=job.sp.kT_quench, n_steps=2e7)
            print("-----------------------------")
            print("Finished NVT quench...")
            print("-----------------------------")
