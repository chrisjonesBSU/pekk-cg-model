...
@MyProject.label
def re_sampled(job):
    return job.isfile("re_means.txt")


@MyProject.label
def extra_runs(job):
    return job.doc.extra_runs == 2
...
#        print(job.id)
#        print("-----------------------")
#        print("Running extension simulation...")
#        print("----------------------")
#        print("Creating the system...")
#        print("----------------------")
#        system_parms = system.System(
...
#            shrink_period = None
#        else: # Not restarting job
#            raise ValueError("Restart not found")
#
#        if job.sp.coarse_grain == True:
#            print("----------------------------------------")
#            print("Preparing a coarse-grained simulation...")
...
#        simulation.quench(n_steps=2e8, kT=job.sp.kT_quench)
#        job.doc.extra_runs += 1
#        print("Run finished...")
...
@MyProject.operation
@MyProject.pre(sampled)
@MyProject.post(re_sampled)
def relaxation(job):
    import numpy as np

    from cmeutils.polymers import end_to_end_distance
    from cmeutils.structure import nematic_order_parameter

    print("Calculating end-to-end distances")

    re_avg, re_std, re_distances, vecs = end_to_end_distance(
        job.fn("sim_traj.gsd"),
        head_index=0,
        tail_index=-1,
        start=0,
    ...
    np.savetxt(fname=job.fn("re_means.txt"), X=re_avg)
    np.savetxt(fname=job.fn("re_stds.txt"), X=re_std)
    # Nematic Order Parameter:
    print("Calculating S2 order parameters")
    nem = []
    for v in vecs:
        nem_order = nematic_order_parameter(vectors=v, director=(0,0,1))
        nem.append(nem_order.order)
    np.savetxt(fname=job.fn("nem_order.txt"), X=np.array(nem))
    print("Finished")
